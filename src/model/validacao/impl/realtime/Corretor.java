    ...
    public ValidacaoResult validar() throws Exception{
        try {
            ...
            if (this.valid.getResultado()) {
                return new ValidacaoResult(valid.getNome(), valid.getMensagem(), valid.getResultado(), Boolean.FALSE);
            } else {
                try {
                    corrigir();
                    return new ValidacaoResult(valid.getNome(), fraseCorrecaoOk(), Boolean.FALSE, Boolean.TRUE);
                } catch (FalhaAoCorrigirException e) {
                    return new ValidacaoResult(valid.getNome(), fraseFalhaCorrecao(), Boolean.FALSE, Boolean.FALSE);
                }
            }
        } catch (Exception ex) {
